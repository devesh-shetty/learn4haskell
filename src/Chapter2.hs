...
-}
subList :: Int -> Int -> [a] -> [a]
subList firstIndex lastIndex list
  | firstIndex < 0 || lastIndex < 0 || lastIndex < firstIndex || null list = []
  | otherwise = traverse (lastIndex - firstIndex + 1) (moveTo firstIndex list)        
    where
      moveTo :: Int -> [a] -> [a]
      moveTo index list
        | null list = []
        | index == 0 = list
        | otherwise = moveTo (index - 1) (drop 1 list)
      traverse :: Int -> [a] -> [a]
      traverse count list
        | count == 0 || null list = []
        | otherwise = head list : traverse (count - 1) (drop 1 list)
...
firstHalf l
  | null l = l
  | otherwise = subList 0 (halfIndex (length l)) l
  where
    halfIndex :: Int -> Int
    halfIndex len = (len `div` 2) - 1
...
takeEven l = go 0 [] l
  where
    ...
    go index acc l
      | null l = acc
      | index `mod` 2 == 0 = go (index + 1) (acc ++ (subList 0 0 l)) (tail l)
      | otherwise = go (index + 1) acc (tail l)                                   
...
smartReplicate [] = []
smartReplicate l = 
  concat (map repeatN l)
    where 
      repeatN :: Int -> [Int]
      repeatN n = replicate n n
...
contains el l = filter (\l1 -> elem el l1) l
...
rotate n l
  | n < 0 || null l = []
  | otherwise = subList n (n + length l - 1) (cycle l)
